...
// Функции. С т.зр. Джава скрипта функции - это такой же тип данных, как и строка и всё остальное, а это значит, что функцию можно положить в переменную.
...
// анонимная функция (здесь не вызовется)
() => {
...
const sum = (firstArgument, seconArgument) => {
  console.log(
  ...
  const result = firstArgument + seconArgument;
...
//не старайтесь вызубрить все функции. всё, с чем вы работаете постоянно - вы будете помнить. А так, Дэн - ваш друг, MDN.
...
//в аргументе ev записана вся информация о том, на что конкретно кликнули (в документации есть описание всех этих ивентов, что в них содержится), куда кликнули, координаты мышки и т.п.
//свойство target - это будет конкретно тот элемент, на который кликнули.

//поместим функцию в переменную (как было показано раньше):
const cb = (ev) => {
  let a = ev.target.innerHTML;
  console.log(a);
};
...
// добавим Event Listenet для второй кнопки.
elBtn2.addEventListener("click", cb);

// добавим второй Event Listenet для первой кнопки.
elBtn1.addEventListener("click", (ev) => {
  console.log("evrika!");
});
