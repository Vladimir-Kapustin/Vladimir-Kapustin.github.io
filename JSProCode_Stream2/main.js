...
//при наведении мышки - просто меняет классы. разные классы - разные стили. Менять напрамую CSS-свойства (т.е. без использования добавления-удаления классов, как написано здесь в коде) очень нежелательно, потому, что тогда вы мешаете вместе логику и отображение. А когда вы просто меняете классы и класс соответствует просто какому-то там статусу элемента, то это гораздо получается более удобная система, т.е. не надо привязываться к визуальщине, т.е. вы знаете что есть, допустим, у элемента состояние "active", т.е. вас не должно даже волновать, как этот "active" выглядит, вы просто назначаете класс "active" и там уже верстальщик или вы сами напишете как этот "active" выглядит (там height и т.д.)
