...
const objectWithFunc = {
  ShowName: () => {
    console.log(
      "вызов функции, находящейся внутри объекта и выводящей какой-то текст, например имя..."
    );
  },
};
...
setTimeout(() => {
  console.log("timer! (из 4-го примера)");
...
//пример блокирующих операций: alert - окно с кнопкой ОК; confirm - окно с кнопками YES, NO; promt - тот же alert, но но строкой кода. "Забыть" про них! Их нет! :)
//callback-функция внутри асинхронной функции setTimeout() не может ничего вернуть, т.к. она вернёт это "в никуда" - т.е это может произойти в разное время во время выполнения программы, в зависимости от основного потока, других потоков. Сама же setTimeout() в данном случае возвращает идентификатор таймера, чтобы её можно было отменить, если надо (см. документацию).

// 5-й пример. Асинхронность и Scope:
console.log("");
console.log("5-й пример. Асинхронность и Scope");

console.log("begin1"); //1-й поток

let c = 10;

setTimeout(() => {
  //2-й поток
  console.log("timer: ", c, "(из  5-го примера)"); //будет выведено 12, не смотря на то, что это значение записано в переменную после функции setTimeout - потому, что сначала выполняется 1-й поток, потом выполняется 2-й поток. В нём функция setTimeout. Она смотрит в своём Scope: какое значение у переменной "с" - поскольку 1-й поток полностью выполнился, то значение присвоено 12.
}, 3000);
...
//события - это одна из основных реализаций паттерна Observer. JavaScript без этого паттерна просто не может работать, потому, что всё асинхронно и работает на таких потоках. Сам язык подразумевает, что постоянно параллельно у него работают какие-то вещи.
